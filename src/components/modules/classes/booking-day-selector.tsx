...
export function BookingDaySelector({ day, classes, activeDay, setActiveDay }: BookingDaySelectorProps) {
  ...
  return (
    <section
      onClick={hasClasses ? () => setActiveDay() : undefined}
      className={cn(
        ...
        hasClasses && 'text-accent-700',
        hasClasses && !isActive && 'hover:bg-tertiary/10 hover:text-primary cursor-pointer',
        !hasClasses && !isActive && 'text-gray-300',
        'after:border-t-10 after:border-x-15',
        'xs:after:border-t-14 xs:after:border-x-22',
        isActive && 'bg-tertiary/20 text-primary after:translate-y-0 after:opacity-100',
      )}
    >
