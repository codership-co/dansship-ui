// forked from design-sync lib/css.mjs — rewriteBundleFontFaces's drop-check
// regex false-positived on quoted, already-rewritten ./fonts/ urls
//
...
export function rewriteBundleFontFaces({ out, bundleCss }) {
  ...
  const next = css.replace(/@font-face\s*\{[^}]*\}/gi, (block) => {
    let b = block;
    // Track resolution per-url during the loop rather than re-testing the
    // mutated string afterward: the original post-hoc regex
    // (`/url\(\s*['"]?(?!https?:|data:|\.\/fonts\/)/`) let `['"]?` backtrack
    // to zero-width to dodge its own negative lookahead whenever the url was
    // quoted (single OR double) — so every successfully-rewritten quoted
    // `./fonts/...` url still false-positived as "unresolved" and the whole
    // block got dropped, even though `rewrote` had already incremented.
    ...
    if (unresolved) { dropped++; return '/* @ds-font-face-dropped: unresolvable src */'; }
    return b;
  });
  if (rewrote || dropped) {
